...
// // ====== OpenAI client (có thể không có key) ======
// const openai =
...
//     let gpu = 0;
//     const gm = blob.match(/\b(RTX|GTX)\s?-?\s?(\d{3,4})\b/);
//     if (gm) {
//         const n = parseInt(gm[2], 10);
//         gpu = n + (gm[1] === "RTX" ? 200 : 0);
//     } else if (/IRIS\s?XE/.test(blob)) gpu = 100;

//     const cpuStr = (p.cpu || "").toUpperCase();
//     let cpu = /I9/.test(cpuStr) ? 900 : /I7/.test(cpuStr) ? 700 : /I5/.test(cpuStr) ? 500 : /I3/.test(cpuStr) ? 300 : 0;
//     const gen = cpuStr.match(/(\d{3,5})/);
//     if (gen) cpu += Math.min(200, Math.floor(parseInt(gen[1], 10) / 10));

//     const ram = +p.ramGB || 0;
//     const ssd = +p.storageGB || 0;
//     const screenBonus = /QHD|OLED|RETINA|144HZ|120HZ/.test(blob) ? 20 : 0;
...
//     if (rng) {
//         minBudget = +rng[1] * 1_000_000;
//         maxBudget = +rng[2] * 1_000_000;
//     } else {
...
//         else if (about) {
//             const v = +about[1] * 1_000_000;
//             minBudget = Math.max(0, v - 3_000_000);
//             maxBudget = v + 3_000_000;
//         } else if (one) {
//             const v = +one[1] * 1_000_000;
//             minBudget = Math.max(0, v - 3_000_000);
//             maxBudget = v + 3_000_000;
//         }
//     }
...
// /* -------------------- Lưu / đọc filter ngắn hạn theo session -------------------- */
// async function setEphemeralFilter(
//     sessionId: number,
...
//             where: { id: exist.id },
//             data: { value, type: "EPHEMERAL", expiresAt, score: 0.5 },
//         });
//     } else {
//         await prisma.aiMemory.create({
//             data: { sessionId, key, value, type: "EPHEMERAL", score: 0.5, expiresAt },
//         });
//     }
// }
...
//         orderBy: { id: "desc" },
//     });
//     const out: { brand?: string; target?: string; min?: number; max?: number } = {};
//     for (const m of mems) {
...
//         (a, b) => scoreProduct(b) - scoreProduct(a) || finalPrice(a) - finalPrice(b)
//     );
...
//                     where: { userId: userId ?? undefined, status: "OPEN" },
...
//             await setEphemeralFilter(
//                 session.id,
//                 "filter.budget",
//                 JSON.stringify({ min: minBudget, max: maxBudget })
//             );
//         }
...
//                 const reply = [
//                     `Mạnh nhất trong bộ lọc hiện tại (${remembered.brand || "mọi hãng"}${remembered.target ? ` / ${remembered.target}` : ""}${remembered.min || remembered.max ? ` — ${(remembered.min || 0) / 1e6}–${typeof remembered.max === "number" ? remembered.max / 1e6 : "∞"}tr` : ""
//                     }):`,
//                     `• **${p.name}** — ${sale.toLocaleString("vi-VN")}₫${p.discount ? ` (giảm ${p.discount}%, gốc ${base.toLocaleString("vi-VN")}₫)` : ""
//                     } → /product/${p.id}`,
//                     `• CPU: ${p.cpu || "—"}, RAM: ${p.ramGB || "?"}GB, Ổ: ${p.storageGB || "?"}GB ${p.storageType || ""}`,
//                     `• Màn: ${p.screenSizeInch || "?"}" ${p.screenResolution || ""} | Tag: ${p.featureTags || "—"}`,
//                 ].join("\n");
...
//                 return res.json({ sessionId: session.id, reply, products: [productDTO(p)] });
//             }
//         }
...
//             if (list.length) {
//                 const legend = `Gợi ý theo ${remembered.brand || "tất cả hãng"}${remembered.target ? ` / ${remembered.target}` : ""}${remembered.min || remembered.max
//                     ? ` — ${(remembered.min || 0) / 1e6}–${typeof remembered.max === "number" ? remembered.max / 1e6 : "∞"}tr`
//                     : ""
//                     }:`;
//                 const lines = list.map(formatListLine).join("\n");
//                 const reply = `${legend}\n${lines}\n\nBạn muốn mình **chọn mạnh nhất** trong số này không?`;
...
//                 return res.json({ sessionId: session.id, reply, products: list.map(productDTO) });
//             }
//         }

//         // --- Nếu không rơi vào 2 intent trên: thử LLM, nếu fail -> fallback DB
//         let reply = "";
//         try {
...
//                 content:
//                     "Bạn là **Rùa AI** của LaptopShop. Trả lời ngắn gọn, có gạch đầu dòng khi cần. Nếu nhắc đến sản phẩm trong cửa hàng, luôn gắn link /product/:id. Ngôn ngữ: tiếng Việt.",
//             };
//             const msgs = [
//                 sys,
...
//         } catch (e: any) {
//             // Fallback (hết quota / không có key / lỗi model)
...
//                         ? `• Bộ lọc: ${remembered.brand || "tất cả hãng"}${remembered.target ? ` / ${remembered.target}` : ""}${remembered.min || remembered.max
//                             ? ` — ${(remembered.min || 0) / 1e6}–${typeof remembered.max === "number" ? remembered.max / 1e6 : "∞"}tr`
//                             : ""
//                         }`
...
//             } else {
//                 reply =
//                     "Hiện chưa truy xuất được AI. Bạn cho mình thêm **hãng/nhu cầu/ngân sách** (vd: ASUS gaming 20–25 triệu), mình lọc sản phẩm có link ngay nhé!";
//             }
//         }
...
//         return res
//             .status(200)
//             .json({ reply: "Có lỗi máy chủ. Bạn thử lại giúp mình nhé.", error: String(err?.message || err) });
//     }
// });
